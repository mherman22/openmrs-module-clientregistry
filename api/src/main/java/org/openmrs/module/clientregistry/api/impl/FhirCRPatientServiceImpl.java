...
public class FhirCRPatientServiceImpl implements CRPatientService {
	...
	public IBundleProvider getPatientsByPIX(String sourceIdentifier, String sourceIdentifierSystem,
		...
		List<String> crIdentifiers = crMatchingParams.getParameter().stream().filter(param -> Objects.equals(param.getName(),
						"targetId")).map(param -> ((Reference) param.getValue()).getReference()).collect(Collectors.toList());
